...
function findFirstSingleChar(str) {
  const ans = str.replace(/ /g, '').split('').reduce((a, b, i, arr) => {
    const r = a;
    if (i === 0) {
      if (!arr.slice(1).includes(b)) a.push(b);
    } else if (!arr.slice(i + 1).includes(b) && !arr.slice(0, i).includes(b)) r.push(b);
    return r;
  }, []);
  return ans[0] ? ans[0] : null;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const begin = `${isStartIncluded ? '[' : '('}${Math.min(a, b)}, `;
  const end = `${Math.max(a, b)}${isEndIncluded ? ']' : ')'}`;
  return begin + end;
}
 ...
 * and false otherwise.
 *
 * See algorithm here : https://en.wikipedia.org/wikixcvby/Luhn_algorithm
 *
 * @param {number} cnn
 * @return {boolean}
 *
...
function getDigitalRoot(num) {
  const arr = num.toString().split('').map((el) => el * 1);
  const pocNum = arr.reduce((a, b) => a + b);
  return pocNum < 10 ? pocNum : getDigitalRoot(pocNum);
}
...
function evaluateTicTacToePosition(position) {
  ...
  for (let i = 0; i < 3; i += 1) {
    ...
    for (let k = 0; k < 3; k += 1) {
      l[i].push(position[i][k] || 'Z');
    }
    for (let k = 0; k < 3; k += 1) {
      columns[i].push(position[k][i] || 'Z');
    }
  }
  const a = l.concat(columns).concat([dig1, dig2]).filter((el) => (el[0] !== 'Z' && el[1] !== 'Z' && el[2] !== 'Z'));
  const won = a.filter((el) => (el[0] === el[1] && el[1] === el[2] && el[0] === el[2]))[0];
  if (won) {
    return won[0];
  }
  return won;
